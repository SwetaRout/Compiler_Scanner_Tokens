...
public class RegularExpressions {
	...
	public boolean isIdentifier(String str) {	
		...
		//Checks that if the token starts with an alphabet
		if((ch >=65 && ch<=90)||(ch>=97 && ch<=122)|| ch == 95)
		{
			//Checks if all other characters of the token are either letter or digit or underscore
			for(int i=1;i<str.length();i++)
			{
				char ch1=str.charAt(i);
				if((ch1>=65 && ch1<=90) || (ch1>=97 && ch1<=122) || (ch1>=48 && ch1<=57) || ch1== 95)
				{
					;
				}
				else
